...
<head>
    ...
    <title>Top Up Free Fire - Market.in</title>
        ...
        <div class="max-w-[1200px] mx-auto flex flex-wrap justify-between items-center gap-4">
            ...
            <nav class="flex items-center">
                <ul class="flex items-center gap-6 m-0 p-0 list-none">
                    <li><a href="index.php" class="text-sm font-semibold text-zinc-400 hover:text-[#f3af22]">Home</a></li>
                    <li><a href="daftar-game.php" class="text-sm font-semibold text-zinc-400 hover:text-[#f3af22]">Daftar Game</a></li>
    ...
    <main class="max-w-6xl mx-auto px-4 py-8 pt-28">
        ...
        <div class="bg-gradient-to-r from-zinc-900/90 to-black/90 backdrop-blur-md rounded-xl p-8 mb-8 flex flex-col md:flex-row justify-between items-center gap-6 border border-zinc-800">
            <div class="flex items-center gap-6">
                <div class="w-24 h-24 md:w-32 md:h-32 bg-zinc-800 rounded-xl overflow-hidden flex items-center justify-center font-bold text-zinc-600">
                    <img src="img/logo-ff.png" alt="Free Fire" class="w-24 h-24 md:w-32 md:h-32 rounded-xl object-cover">
                </div>
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold tracking-wide">FREE FIRE</h1>
                    <p class="text-zinc-400 text-sm mt-1">Garena Free Fire • Garena</p>
                </div>
            </div>
            <div class="text-right hidden md:block">
                <h2 class="text-3xl font-black text-gold tracking-wider">TOP UP</h2>
                <h2 class="text-2xl font-bold text-zinc-300">FREE FIRE</h2>
            </div>
        </div>

        <form action="pembayaran.php" method="POST" id="formTopup" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <!-- ID Game disesuaikan dengan database Free Fire Anda (yaitu 6) -->
            <input type="hidden" name="id_game" value="3">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80">
                    ...
                    <!-- 1. BAGIAN MEMBERSHIP (DINAMIS BACA DATABASE) -->
                    <div class="mb-8">
                        <h4 class="text-lg font-serif italic font-semibold text-zinc-200 mb-4 tracking-wide">Membership Free Fire</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <?php
                            $id_game_ff = 3;
                            // Menarik item khusus yang mempunyai teks 'Membership' di tabel game_items
                            $query_membership = mysqli_query($koneksi, "SELECT * FROM game_items WHERE id_game = $id_game_ff AND nama_item LIKE '%Membership%' AND status = 'Tersedia' ORDER BY harga_jual ASC");

                            if ($query_membership && mysqli_num_rows($query_membership) > 0) {
                                while ($ms = mysqli_fetch_assoc($query_membership)) {
                                    echo '
                                    <label class="cursor-pointer group relative">
                                        <input type="radio" name="nominal" value="'.$ms['nama_item'].'" data-price="'.(int)$ms['harga_jual'].'" class="hidden peer" required>
                                        <div class="bg-zinc-900/40 backdrop-blur-sm border border-zinc-800/60 p-5 rounded-xl text-left transition duration-200 peer-checked:border-gold peer-checked:bg-zinc-800/50 group-hover:border-zinc-600">
                                            <div class="text-sm font-medium text-zinc-300 group-hover:text-white transition">'.htmlspecialchars($ms['nama_item']).'</div>
                                            <div class="text-sm font-normal text-zinc-400 mt-2">Rp '.number_format($ms['harga_jual'], 0, ',', '.').'</div>
                                        </div>
                                    </label>';
                                }
                            } else {
                                echo '<div class="text-zinc-500 text-xs col-span-2 py-4">Belum ada varian Membership yang tersedia di database.</div>';
                            }
                            ?>
                        </div>
                    </div>

                    <!-- 2. BAGIAN DIAMOND UTAMA (DINAMIS BACA DATABASE) -->
                    <div>
                        <h4 class="text-lg font-serif italic font-semibold text-zinc-200 mb-4 tracking-wide">Top Up FF Murah</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <?php
                            // Menarik item yang murni Diamond biasa (TIDAK mengandung kata 'Membership')
                            $query_items = mysqli_query($koneksi, "SELECT * FROM game_items WHERE id_game = $id_game_ff AND nama_item NOT LIKE '%Membership%' AND status = 'Tersedia' ORDER BY harga_jual ASC");
                            ...
                            if ($query_items && mysqli_num_rows($query_items) > 0) {
                                ...
                                echo '<div class="text-zinc-500 text-xs col-span-2 py-4 text-center">Belum ada varian Diamond FF yang tersedia di database.</div>';
                            }
                            ?>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80 space-y-4">
                    ...
                    <div>
                        <label class="text-xs text-zinc-400 block mb-1">Masukkan Player ID</label>
                        <input type="number" name="user_id" placeholder="Contoh: 123456789" class="w-full bg-[#1a1a1a]/80 border border-zinc-800 rounded px-3 py-2 text-sm focus:outline-none focus:border-gold" required>
                    </div>
                ...
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="<?= htmlspecialchars($email); ?>" placeholder="Masukkan email" class="w-full bg-transparent border border-zinc-600 rounded-xl px-4 py-2.5 text-sm placeholder-zinc-500 focus:outline-none focus:border-zinc-400" required>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
            <div class="bg-card p-6 rounded-xl border border-zinc-800 text-center space-y-2">
                ...
                <p class="text-xs text-zinc-400">Pesanan Anda diproses secara otomatis dalam hitungan detik setelah pembayaran dikonfirmasi.</p>
            </div>
            <div class="bg-card p-6 rounded-xl border border-zinc-800 text-center space-y-2">
