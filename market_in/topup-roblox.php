<?php
session_start();
include 'koneksi.php'; // Menyertakan koneksi database agar query berfungsi
...
<head>
    ...
    <title>Beli Roblox Gift Card - Market.in</title>
...
<body>
        ...
        <div class="max-w-[1200px] mx-auto flex flex-wrap justify-between items-center gap-4">
            ...
            <nav class="flex items-center">
                <ul class="flex items-center gap-6 m-0 p-0 list-none">
                    <li><a href="index.php" class="text-sm font-semibold text-zinc-400 hover:text-[#f3af22] transition-colors">Home</a></li>
                    <li><a href="daftar-game.php" class="text-sm font-semibold text-zinc-400 hover:text-[#f3af22] transition-colors">Daftar Game</a></li>
                    ...
                    <?php else : ?>
                        <li><a href="login.php" class="border border-[#f3af22] text-[#f3af22] px-4 py-1.5 rounded-full text-sm font-medium hover:bg-[#f3af22] hover:text-black transition">Masuk</a></li>
                    <?php endif; ?>
                </ul>
            </nav>
        </div>
    ...
    <main class="max-w-6xl mx-auto px-4 py-8 pt-28">
        ...
        <div class="bg-gradient-to-r from-zinc-900/90 to-black/90 backdrop-blur-md rounded-xl p-8 mb-8 flex flex-col md:flex-row justify-between items-center gap-6 border border-zinc-800">
            <div class="flex items-center gap-6">
                <div class="w-24 h-24 md:w-32 md:h-32 bg-zinc-800 rounded-xl overflow-hidden flex items-center justify-center font-bold text-zinc-600">
                    <img src="img/logo-roblox.webp" alt="Roblox" class="w-24 h-24 md:w-32 md:h-32 rounded-xl object-cover">
                </div>
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold tracking-wide">ROBLOX GIFT CARD</h1>
                    <p class="text-zinc-400 text-sm mt-1">Roblox Digital Code • Roblox Corporation</p>
                </div>
            </div>
            <div class="text-right hidden md:block">
                <h2 class="text-3xl font-black text-gold tracking-wider">BELI</h2>
                <h2 class="text-2xl font-bold text-zinc-300">GIFT CARD</h2>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80">
                    ...
                    <div class="flex items-center gap-2 mb-6">
                        ...
                        <h3 class="font-semibold text-xl tracking-wide text-zinc-100">Pilih Nominal Voucher</h3>
                    </div>

                    <div>
                        <h4 class="text-lg font-serif italic font-semibold text-zinc-200 mb-4 tracking-wide">Roblox Gift Card</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <?php
                            // 🎮 SINKRONISASI DATABASE ADMIN: Mengambil produk Roblox dari tabel game_items
                            // ID Game untuk Roblox adalah 4, hanya memanggil yang statusnya 'Tersedia'
                            $id_game_roblox = 4; 
                            $query_items = mysqli_query($koneksi, "SELECT * FROM game_items WHERE id_game = $id_game_roblox AND status = 'Tersedia' ORDER BY harga_jual ASC");
                            ...
                            if ($query_items && mysqli_num_rows($query_items) > 0) {
                                ...
                                echo '<div class="text-zinc-500 text-xs col-span-2 py-4 text-center">Belum ada varian Roblox Gift Card yang di-input di Panel Admin Kelola Item.</div>';
                            }
                            ?>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80 space-y-4">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-lg">Masukkan Data Penerima</h3>
                    </div>
                    <div>
                        <label class="text-xs text-zinc-400 block mb-1">Nama Pengguna / WhatsApp No.</label>
                        <input type="text" name="user_id" placeholder="Contoh: RobloxUser / 0812xx" class="w-full bg-[#1a1a1a]/80 border border-zinc-800 rounded px-3 py-2 text-sm focus:outline-none focus:border-gold text-white" required>
                    </div>
                ...
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80 space-y-3">
                    ...
                    <div>
                        <input type="text" name="kode_promo" id="kode_promo" placeholder="Masukkan kode promo" class="w-full bg-transparent border border-zinc-600 rounded-xl px-4 py-2.5 text-sm placeholder-zinc-500 focus:outline-none focus:border-zinc-400 text-white">
                    </div>
                ...
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80 space-y-4">
                    ...
                    <div>
                        <label class="text-sm text-zinc-300 block mb-2 font-medium">Email Pengiriman</label>
                        <input type="email" name="email" value="<?= htmlspecialchars($email); ?>" placeholder="Masukkan email" class="w-full bg-transparent border border-zinc-600 rounded-xl px-4 py-2.5 text-sm placeholder-zinc-500 focus:outline-none focus:border-zinc-400 text-white" required>
                    </div>
                    <div class="flex items-start gap-2 text-xs text-zinc-500 bg-zinc-900/40 p-3 rounded-lg border border-zinc-800/50 leading-relaxed">
                        ...
                        <span>Kode voucher Gift Card akan dikirim langsung ke alamat email ini setelah status sukses.</span>
                    </div>

                    <input type="hidden" name="total" id="total_harga" value="0">
                    <input type="hidden" name="id_game" value="4"> 
                    <input type="hidden" name="order_id" value="<?= 'INV/'.date('Ymd').'/'.rand(10000, 99999); ?>">
                    ...
                    <?php else : ?>
                        <a href="login.php" class="w-full mt-4 block text-center bg-zinc-800 border border-zinc-700 text-zinc-400 font-bold py-3.5 rounded-xl uppercase tracking-wider text-sm hover:bg-zinc-700 hover:text-white transition">
                            🔒 Login Terlevelih Dahulu Untuk Membeli
                        </a>
                    <?php endif; ?>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
            <div class="bg-card p-6 rounded-xl border border-zinc-800 text-center space-y-2">
                <div class="text-gold text-2xl">⚡</div>
                <h4 class="font-bold text-gold">Pengiriman Instan</h4>
                <p class="text-xs text-zinc-400">Kode Gift Card otomatis terbit dan dikirim langsung ke email Anda setelah verifikasi dana berhasil.</p>
            </div>
            <div class="bg-card p-6 rounded-xl border border-zinc-800 text-center space-y-2">
                <div class="text-gold text-2xl">🛡️</div>
                <h4 class="font-bold text-gold">Voucher Resmi & Valid</h4>
                <p class="text-xs text-zinc-400">Jaminan 100% kode valid langsung dari distributor utama Roblox Corporation, aman untuk di-redeem.</p>
            </div>
            <div class="bg-card p-6 rounded-xl border border-zinc-800 text-center space-y-2">
                <div class="text-gold text-2xl">🎧</div>
                <h4 class="font-bold text-gold">Layanan CS 24/7</h4>
                <p class="text-xs text-zinc-400">Tim customer service kami siap membantu kendala penukaran kode voucher Anda kapan saja.</p>
            </div>
        </div>
