...
<head>
    ...
    <title>Top Up PUBG Mobile - Market.in</title>
        ...
        <div class="bg-gradient-to-r from-zinc-900/90 to-black/90 backdrop-blur-md rounded-xl p-8 mb-8 flex flex-col md:flex-row justify-between items-center gap-6 border border-zinc-800">
            <div class="flex items-center gap-6">
                <div class="w-24 h-24 md:w-32 md:h-32 bg-zinc-800 rounded-xl overflow-hidden flex items-center justify-center font-bold text-zinc-600">
                    <img src="img/logo-pubg.jpg" alt="PUBG Mobile" class="w-24 h-24 md:w-32 md:h-32 rounded-xl object-cover">
                </div>
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold tracking-wide">PUBG MOBILE</h1>
                    <p class="text-zinc-400 text-sm mt-1">PUBG Mobile • Tencent Games</p>
                </div>
            </div>
            <div class="text-right hidden md:block">
                <h2 class="text-3xl font-black text-gold tracking-wider">TOP UP</h2>
                <h2 class="text-2xl font-bold text-zinc-300">PUBG MOBILE</h2>
            </div>
        </div>
        ...
        <form action="pembayaran.php" method="POST" id="formTopup" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80">
                    ...
                    <div>
                        <h4 class="text-lg font-serif italic font-semibold text-zinc-200 mb-4 tracking-wide">Top Up Unknown Cash (UC)</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <?php
                            // 🎮 SINKRONISASI DATABASE: Mengambil data UC dari tabel game_items
                            // ID Game untuk PUBG Mobile adalah 2, hanya menampilkan yang berstatus 'Tersedia'
                            $id_game_pubg = 2; 
                            $query_items = mysqli_query($koneksi, "SELECT * FROM game_items WHERE id_game = $id_game_pubg AND status = 'Tersedia' ORDER BY harga_jual ASC");
                            ...
                            if ($query_items && mysqli_num_rows($query_items) > 0) {
                                ...
                                echo '<div class="text-zinc-500 text-xs col-span-2 py-4 text-center">Belum ada varian UC yang dimasukkan di Panel Admin Kelola Item.</div>';
                            }
                            ?>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80 space-y-4">
                    ...
                    <div>
                        <label class="text-xs text-zinc-400 block mb-1">Masukkan ID Karakter (UID)</label>
                        <input type="number" name="user_id" placeholder="Contoh: 512345678" class="w-full bg-[#1a1a1a]/80 border border-zinc-800 rounded px-3 py-2 text-sm focus:outline-none focus:border-gold text-white" required>
                    </div>
                ...
                <div class="bg-card p-6 rounded-xl border border-zinc-800/80 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="<?php echo htmlspecialchars($email); ?>" placeholder="Masukkan email" class="w-full bg-transparent border border-zinc-600 rounded-xl px-4 py-2.5 text-sm placeholder-zinc-500 focus:outline-none focus:border-zinc-400 text-white" required>
                    </div>
                    ...
                    <input type="hidden" name="id_game" value="2">
